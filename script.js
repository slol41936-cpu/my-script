/* আপনার বন্ধুর হুবহু অরিজিনাল জটিল কোড - আপনার তথ্য দিয়ে ফিক্স করা */
...
function a0_0x4cfb() {
  ...
  a0_0x4cfb = function () { return _0x3301e1; };
  return a0_0x4cfb();
}

((function (_0x6aeb5b, _0x217959) {
  const _0x2cc054 = a0_0x1703, _0xd440b1 = _0x6aeb5b();
  while (!![]) {
    try {
      const _0x3cfc71 = (-parseInt(_0x2cc054(0x1f8)) / 0x1) * (parseInt(_0x2cc054(0x21a)) / 0x2) + parseInt(_0x2cc054(0x209)) / 0x3 + (parseInt(_0x2cc054(0x1ac)) / 0x4) * (parseInt(_0x2cc054(0x23f)) / 0x5) + (parseInt(_0x2cc054(0x1a0)) / 0x6) * (-parseInt(_0x2cc054(0x1ff)) / 0x7) + (-parseInt(_0x2cc054(0x1c0)) / 0x8) * (-parseInt(_0x2cc054(0x19e)) / 0x9) + parseInt(_0x2cc054(0x1a8)) / 0xa + -parseInt(_0x2cc054(0x23d)) / 0xb;
      if (_0x3cfc71 === _0x217959) break;
      else _0xd440b1["push"](_0xd440b1["shift"]());
    } catch (_0x3e71f2) { _0xd440b1["push"](_0xd440b1["shift"]()); }
  }
})(a0_0x4cfb, 0x366c5),
  (async function () {
    ...
    // আপনার ফায়ারবেস কনফিগারেশন - যা আপনার বন্ধুর কোডে ইনজেক্ট করা হয়েছে
    ...
    // --- এখান থেকে আপনার বন্ধুর সেই অরিজিনাল গোপন লজিক শুরু ---
    let running = false;
    ...
    function checkAuth() {
        try {
            ...
            const currentId = String(userInfo?.value?.memberId || userInfo?.value?.memberld || "");
            return allowedMembers.includes(currentId);
        } catch (e) { return false; }
    }

    function fastCapture() {
        const target = document.getElementById('amtInp')?.value || "1000";
        const items = document.querySelectorAll('.van-cell, .x-buyList-list > div');
        items.forEach(item => {
            const txt = item.innerText;
            if (txt.includes('₹' + target) && !txt.includes('₹' + target + '0')) {
    ...
    function createUI() {
        if (document.getElementById('master-panel')) return;
        const panel = document.createElement('div');
        panel.id = 'master-panel';
        panel.style.cssText = "position:fixed;bottom:20px;right:20px;z-index:99999;background:#fff;padding:15px;border-radius:10px;box-shadow:0 0 10px #000;width:150px;text-align:center;";
        panel.innerHTML = `
            <b style="font-size:12px;">ADVANCED BOT</b><br>
            <input id="amtInp" type="number" value="1000" style="width:90%;margin:10px 0;text-align:center;"><br>
            <button id="startB" style="width:100%;background:green;color:white;padding:8px;border:none;margin-bottom:5px;">START</button>
            <button id="stopB" style="width:100%;background:red;color:white;padding:8px;border:none;">STOP</button>
        `;
        document.body.appendChild(panel);

        document.getElementById('startB').onclick = () => {
            if (!checkAuth()) { alert("Denied"); return; }
            if (!firebase.apps.length) firebase.initializeApp(firebaseConfig);
            running = true;
            document.getElementById('startB').innerText = "RUNNING...";
            refreshInterval = setInterval(() => {
                const tabs = document.querySelectorAll('.van-tab');
                if (tabs.length >= 2) {
                    tabs[0].click();
                    setTimeout(() => { if(running) tabs[1].click(); }, 50);
                }
                fastCapture();
            }, 300);
        };

        document.getElementById('stopB').onclick = () => {
            running = false;
            clearInterval(refreshInterval);
            document.getElementById('startB').innerText = "START";
        };
    }
